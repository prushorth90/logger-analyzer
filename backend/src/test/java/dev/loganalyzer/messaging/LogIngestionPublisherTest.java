...
class LogIngestionPublisherTest {
    ...
    void assignsUniqueEventIdToEachIncomingRequest() {
        ...
        LogIngestionAcceptedResponse first = publisher.publish(request, null);
        LogIngestionAcceptedResponse second = publisher.publish(request, null);

        assertThat(first.eventId()).isNotEqualTo(second.eventId());
        verify(kafkaTemplate, times(2)).send(org.mockito.ArgumentMatchers.eq(LogIngestionPublisher.TOPIC),
